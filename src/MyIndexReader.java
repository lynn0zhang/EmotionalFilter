...
public class MyIndexReader {
    ...
    private ArrayList<Integer> nullContentList = new ArrayList<>();
    ...
    public int docLength(int docid) throws IOException {
        ...
        if (vector == null) {
            if (nullContentList.contains(docid))
                nullContentList.add(docid);
            return 0;
        } else {
    ...
    public int totalWord() throws IOException {
        int totalWord = 0, docnum = 2226;
        for (int i = 0; i < docnum; i++) {
            totalWord += docLength(i);
        }
        return totalWord;
    }
}
